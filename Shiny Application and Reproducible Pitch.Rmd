...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            #sliderInput('mpg', 'Miles/(US) gallon', min=0, max=40, value=c(0,40), step=10), 
...
server <- function(input, output) {
    ...
    output$table <- renderDataTable({
        #mpg_seq <- seq(from = input$mpg[1], to = input$mpg[2], by = 1)
        ...
        #mydata <- filter(mydata, Transmission %in% input$am, NumCylinders %in% input$cyl)
